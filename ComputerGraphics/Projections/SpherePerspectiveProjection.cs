...
public static class SpherePerspectiveProjection
{
    ...
    private const float ViewportWidth = 2f;
    private const float DistanceFromOriginToViewport = 1f;
    
    public static void Draw(ICanvas canvas, Point3D origin, SpheresScene scene)
    {
        ...
        Parallel.For(0, chunks, chunk =>
        {
            ...
            for (int x = xMin; x < xMax; x++)
            {
                for (int y = -canvas.Height / 2 + 1; y < canvas.Height / 2; y++)
                {
                    ...
                    var color = TraceRay(scene, origin, directionVector, 1f, float.PositiveInfinity);
